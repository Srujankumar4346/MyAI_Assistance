...
def google_login(db: Session = Depends(get_db)):
    if not settings.GOOGLE_CLIENT_ID:
        ...
        redirect_target = _get_or_create_social_user(
            db, "google_user@gmail.com", "Google User", "google", "demo_google_id"
        )
...
def github_login(db: Session = Depends(get_db)):
    if not settings.GITHUB_CLIENT_ID:
        ...
        redirect_target = _get_or_create_social_user(
            db, "github_user@github.com", "GitHub User", "github", "demo_github_id"
        )
        return RedirectResponse(redirect_target)
...
def facebook_login(db: Session = Depends(get_db)):
    if not settings.FACEBOOK_CLIENT_ID:
        ...
        redirect_target = _get_or_create_social_user(
            db, "facebook_user@facebook.com", "Facebook User", "facebook", "demo_facebook_id"
        )
        return RedirectResponse(redirect_target)
...
def apple_login(db: Session = Depends(get_db)):
    if not settings.APPLE_CLIENT_ID:
        ...
        redirect_target = _get_or_create_social_user(
            db, "apple_user@icloud.com", "Apple User", "apple", "demo_apple_id"
        )
        return RedirectResponse(redirect_target)
