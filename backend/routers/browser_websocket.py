...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, token: str, workspace: str, db: Session):
        ...
        if not token:
            self.logger.warning(
                f"Security Alert: Rejected WS connection missing token (IP: {websocket.client.host})"
            )
        ...
        if not user:
            self.logger.warning(
                f"Security Alert: Rejected WS connection with invalid/expired token (IP: {websocket.client.host})"
            )
        ...
        if not workspace:
            self.logger.warning(
                f"Security Alert: Rejected WS connection missing workspace (User: {user.id})"
            )
        ...
        self.logger.info(
            f"WebSocket Client Connected: User {user.id} | Workspace {workspace} | IP {websocket.client.host}"
        )
        return True
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        self.logger.info(
            f"WebSocket Client Disconnected: User {meta.get('user_id')} | Workspace {meta.get('workspace')}"
        )
    ...
    async def _handle_bus_event(self, event_type: str, data: dict):
        ...
        if "SESSION" in event_type:
            ...
            k in event_type
            for k in ["MEMORY", "KNOWLEDGE", "LEARNING", "DOCUMENT", "RESEARCH_INGESTED"]
        ):
...
async def websocket_browser_endpoint(
    websocket: WebSocket, token: str = None, workspace: str = None, db: Session = Depends(get_db)
):
