...
async def execute_desktop_action(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid action_type enum value: {request.action_type}"
        )

    executor = desktop_executors.get(request.action_type)
    if not executor:
        raise HTTPException(
            status_code=400, detail=f"Unknown executor for action: {request.action_type}"
        )
...
async def create_macro(
    ...
    macro = macro_engine.save_macro(
        db, current_user.id, request.name, request.trigger_phrase, request.steps
    )
...
async def execute_macro(
    macro_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
