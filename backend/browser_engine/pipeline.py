...
class BrowserPipeline:
    ...
    async def execute_action(
        ...
        perm_status = browser_permission_engine.check_permission(
            db, user_id, action_type, target, level
        )
        ...
        if action_type in [ActionType.BROWSER_NAVIGATION, ActionType.BROWSER_DOWNLOAD]:
            ...
            if not is_safe:
                return self._fail(
                    user_id, action_type, target, f"Safety Validator blocked: {reason}"
                )
    ...
    def _fail(
        self, user_id: int, action_type: ActionType, target: str, reason: str
    ) -> Dict[str, Any]:
