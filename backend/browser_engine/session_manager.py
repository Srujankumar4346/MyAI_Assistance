...
class BrowserSessionManager:
    ...
    def _transition_state(self, new_state: SessionState, reason: str = "", **kwargs):
        ...
        if old_state == SessionState.CLOSED and new_state not in [SessionState.INITIALIZING, SessionState.RESTORING]:
    ...
    async def create_session(self, workspace_id: str) -> str:
        ...
        event_bus.publish("SESSION_CREATED", {"session_id": session_id, "workspace_id": workspace_id})
    ...
    async def save_session(self, session_id: str):
        ...
        event_bus.publish("SESSION_SAVED", {"session_id": session_id, "data": self.active_sessions[session_id]})
        
    async def restore_session(self, session_id: str):
        """Restores a session from persistence. (Delegated)"""
        self._transition_state(SessionState.RESTORING, f"Restoring session {session_id}", session_id=session_id)
