...
class BookmarkManager:
    ...
    def create_bookmark(self, db: Session, workspace_id: int, user_id: int, url: str, title: str, category: str = "Uncategorized", tags: str = "", notes: str = "", is_pinned: bool = False, is_favorite: bool = False):
        bm = Bookmark(
    ...
    def get_bookmarks(self, db: Session, user_id: int, workspace_id: int = None, search: str = None):
        query = db.query(Bookmark).filter_by(user_id=user_id)
        if workspace_id:
            query = query.filter_by(workspace_id=workspace_id)
        
        if search:
            query = query.filter(Bookmark.title.ilike(f"%{search}%") | Bookmark.url.ilike(f"%{search}%") | Bookmark.tags.ilike(f"%{search}%"))
