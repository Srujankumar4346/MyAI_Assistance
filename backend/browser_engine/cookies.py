...
class CookieManager:
    ...
    def save_cookies(self, db: Session, workspace_id: int, user_id: int, domain: str, cookies: list):
        ...
        existing = db.query(BrowserCookie).filter_by(
            workspace_id=workspace_id, user_id=user_id, domain=domain
        ).first()
    ...
    def load_cookies(self, db: Session, workspace_id: int, user_id: int, domain: str) -> list:
        ...
        record = db.query(BrowserCookie).filter_by(
            workspace_id=workspace_id, user_id=user_id, domain=domain
        ).first()
        ...
        # Filter expired
        now = datetime.now(timezone.utc).timestamp()
        valid_cookies = [c for c in cookies if c.get("expires", -1) == -1 or c.get("expires", -1) >= now]
