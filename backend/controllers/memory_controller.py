...
from backend.database.models import User, MemoryModel
...
def get_all_memories_controller(db: Session, current_user: User):
    ...
    return [{"id": m.id, "content": m.content, "category": m.category or "general"} for m in db_memories]
...
def add_memory_controller(payload: MemoryCreateInput, db: Session, current_user: User):
    ...
    success = memory_manager.add_memory(content=payload.content, category=payload.category, metadata={"user_id": current_user.id})
    if not success:
...
def delete_memory_controller(memory_id: str, db: Session, current_user: User):
    ...
    db_mem = db.query(MemoryModel).filter(MemoryModel.id == memory_id, MemoryModel.user_id == current_user.id).first()
    if db_mem:
        db.delete(db_mem)
        db.commit()
