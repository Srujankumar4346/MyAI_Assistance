...
class EnhancedMemory(Base):
    ...
    parent = relationship(
        "EnhancedMemory", remote_side=[id], back_populates="children", foreign_keys=[parent_id]
    )
...
class MemoryRelationship(Base):
    ...
    relationship_type = Column(
        String, default="related_to"
    )  # 'related_to', 'part_of', 'contradicts', 'supports'
...
class KnowledgeNode(Base):
    ...
    node_type = Column(
        String, default="concept"
    )  # 'user', 'project', 'language', 'framework', 'person', 'goal', 'skill', 'concept'
    ...
    outgoing_edges = relationship(
        "KnowledgeEdge", foreign_keys="KnowledgeEdge.source_id", cascade="all, delete-orphan"
    )
    incoming_edges = relationship(
        "KnowledgeEdge", foreign_keys="KnowledgeEdge.target_id", cascade="all, delete-orphan"
    )
...
class KnowledgeEdge(Base):
    ...
    relationship = Column(
        String, default="related_to"
    )  # 'uses', 'part_of', 'knows', 'works_on', etc.
...
class LearningEvent(Base):
    ...
    )  # 'new_language', 'new_framework', 'habit_detected', etc.
...
class MemoryTimelineEvent(Base):
    ...
    event_type = Column(
        String, default="memory_created"
    )  # 'memory_created', 'memory_accessed', 'knowledge_added'
