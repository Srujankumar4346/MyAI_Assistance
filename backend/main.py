...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # In production, replace with specific origins from settings
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={
            "detail": "An internal server error occurred in NOVA_X OS. Please check logs for details."
        },
    )
...
def read_root():
    return {
        "status": "online",
        "message": f"Welcome to {settings.APP_NAME} Backend API. Visit /docs for documentation.",
    }
