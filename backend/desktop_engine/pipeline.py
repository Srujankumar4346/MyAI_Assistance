...
from backend.desktop_engine.permissions import permission_engine, PermissionLevel, ActionType
...
class DesktopPipeline:
    ...
    def execute_action(
        ...
        if action_type == ActionType.FILE_SYSTEM:
            is_safe, reason = safety_validator.validate_filesystem(target, params.get("fs_action", ""))
        ...
        if not is_safe:
            return self._fail(db, user_id, action_type, target, f"Safety Validator blocked: {reason}")
    ...
    def _fail(self, db: Session, user_id: int, action_type: ActionType, target: str, reason: str) -> Dict[str, Any]:
