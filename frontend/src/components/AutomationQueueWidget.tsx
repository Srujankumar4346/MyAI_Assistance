...
export const AutomationQueueWidget: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe('browser.automation', (event: BrowserEvent) => {
      ...
      if (event.event_type === 'AUTOMATION_STARTED') {
        ...
        setQueue(prev => prev.map(a => a.id === payload.action_id ? { ...a, status: 'waiting_permission' } : a));
      } else if (event.event_type === 'AUTOMATION_COMPLETED') {
        setQueue(prev => prev.map(a => a.id === payload.id ? { ...a, status: 'completed' } : a));
  ...
  if (isLoading) {
    return <div className="p-4 bg-gray-900/50 backdrop-blur-md rounded-xl text-gray-400">Loading Automation Queue...</div>;
  }
  ...
  return (
    ...
    >
      <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-black/20">
        ...
        <button onClick={toggleQueue} className="p-1.5 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors text-gray-300">
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-500">{item.workspace} • {item.priority}</span>
                    </div>
                  </div>
                </div>
                ...
                {item.status === 'waiting_permission' && (
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => handlePermission(item.id, true)} className="flex-1 bg-orange-500/20 hover:bg-orange-500/30 text-orange-400 text-xs font-bold py-1.5 rounded transition-colors flex items-center justify-center gap-1">
                      <Check className="w-3 h-3" /> Approve
                    </button>
                    <button onClick={() => handlePermission(item.id, false)} className="flex-1 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-xs font-bold py-1.5 rounded transition-colors flex items-center justify-center gap-1">
