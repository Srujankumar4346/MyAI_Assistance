...
export const MemoryKnowledgeWidget: React.FC = React.memo(() => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'memory': return <BrainCircuit className="w-4 h-4 text-pink-400" />;
      case 'knowledge': return <Share2 className="w-4 h-4 text-blue-400" />;
      case 'learning': return <Zap className="w-4 h-4 text-yellow-400" />;
      case 'document': return <BookOpen className="w-4 h-4 text-green-400" />;
  ...
  if (isLoading) {
    return <div className="p-4 bg-gray-900/50 backdrop-blur-md rounded-xl text-gray-400">Loading Memory Center...</div>;
  }
