  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((item) => (
        <span
          key={item}
          className="text-xs px-2.5 py-1 rounded-full bg-indigo-500/15 text-indigo-300 border border-indigo-500/25 capitalize"
        >
          {item}
        </span>
      ))}
      ...
      <div className="glass-panel p-6 rounded-2xl border border-emerald-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-emerald-300 font-bold text-xl mb-1">
            ...
            <span className="text-[10px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full ml-1">
              P3
            </span>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="text-sm text-white font-mono">
                {profile?.primary_language || 'Not detected'}
              </div>
            </div>
            <div>
      ...
      <div className="glass-panel rounded-2xl border border-white/8 p-6">
        ...
        {editMode ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            />
            <ProfileField
              label="Secondary Languages (comma-sep)"
              value={secondaryLangs}
              onChange={setSecondaryLangs}
            />
            <ProfileField
              label="Preferred Frameworks (comma-sep)"
              value={frameworks}
              onChange={setFrameworks}
            />
            <ProfileField
              label="Preferred AI Models (comma-sep)"
              value={aiModels}
          ...
          {showEvents && (
            ...
            >
              {stats.recent_events.map((e: any, i: number) => (
                <div
                  key={i}
                  className="flex items-start gap-3 p-3 glass-panel rounded-xl border border-white/5"
                >
