...
export const Memory: React.FC = () => {
    ...
    setMemories((prev) =>
      prev.map((m) =>
        m.id === id
          ? {
              ...m,
              importance_score: Math.min(100, m.importance_score + 5),
  ...
  const handleExport = async () => {
    ...
    a.href = url;
    a.download = 'novax_memories.json';
    a.click();
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Semantic memory with importance scoring, decay, reinforcement, and knowledge graph
              integration.
            </p>
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="glass-panel rounded-2xl border border-white/8 p-4 h-40 animate-pulse bg-slate-800/40"
                />
              ))}
            </div>
          ) : memories.length === 0 ? (
        ...
        <div className="space-y-4">
          <div className="glass-panel p-4 rounded-2xl border border-white/10">
            <div className="flex items-center justify-between mb-3">
              ...
              <label
                className={`px-4 py-2 text-xs rounded-xl flex items-center gap-2 cursor-pointer font-medium transition-all ${uploadingDoc ? 'bg-slate-700 text-slate-400' : 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white hover:opacity-90'}`}
              >
            ...
            <p className="text-xs text-slate-400 mb-4">
              Upload PDF, DOCX, PPTX, XLSX, or TXT files. NOVA_X will extract key facts and store
              them as memories.
            </p>
            {documents.length === 0 ? (
              ...
              <div className="space-y-2">
                {documents.map((doc) => (
                  <div
                    key={doc.id}
                    className="flex items-center justify-between p-3 glass-panel rounded-xl border border-white/5"
                  >
                    <div>
                      <div className="text-sm text-white">{doc.filename}</div>
                      <div className="text-xs text-slate-400 mt-0.5">
                        {doc.file_type.toUpperCase()} · {doc.page_count} pages ·{' '}
                        {doc.memories_extracted} memories extracted
                      </div>
                    </div>
                    <span
