      ...
      <div className="glass-panel p-6 rounded-2xl border border-indigo-500/30 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Customize AI model engines, memory behavior, and UI features.
          </p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-6">
        ...
        <div className="space-y-4">
          {[
            {
              key: 'memory_enabled',
              label: 'Long-term Vector Memory Retrieval',
              desc: 'Query ChromaDB context during conversation generation',
            },
            {
              key: 'streaming_enabled',
              label: 'Token Response Streaming',
              desc: 'Stream AI output chunk by chunk in real time',
            },
            {
              key: 'animation_enabled',
              label: 'Glassmorphism & Micro-animations',
              desc: 'Enable glow buttons, pulse metrics, and smooth transitions',
            },
            {
              key: 'auto_save_conversations',
              label: 'Auto-save Chat Sessions',
              desc: 'Persist chat history to SQLite database automatically',
            },
          ].map(({ key, label, desc }) => (
            <div
              key={key}
